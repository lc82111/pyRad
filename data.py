...
class Data():
    ...
    def project_to_validRays_torch(self, dict_fluences):
        ...
        for (beam_id, msk), (_, fluence) in zip(self.dict_rayBoolMat.items(), dict_fluences.items()):
            high, width = msk.shape
            msk = torch.tensor(msk, dtype=torch.bool, device=fluence_map.device)
            fluence_map = fluence.view(high, width)  # vector to 2D matrix
            valid_rays.append(fluence_map[msk].flatten()) # select valid rays and back to 1d vector
            dict_fluenceMaps[beam_id] = fluence_map.detach()
...
class DataBak():
    ...
    def __init__(self, hparam):
        self.hparam = hparam
        cprint('get info from csv file.', 'green')
        self._setup_paramters_from_csv_table()

        if os.path.isdir(self.hparam.CT_RTStruct_dir):
            cprint('get organ 3D bool index from dicom RTStruct.', 'green')
            self._get_organ_3D_index()
        else:
            cprint('RTStruct fold not found.', 'green')
        
        if 'deposition_file' in hparam:
            self.deposition, self.organName_ptsNum = self.get_depositionMatrix_and_organName_ptsNum()
            self.max_ray_idx = self.deposition.shape[1]
            cprint('deposition loaded.', 'green')
        else:
            assert('max_ray_idx' in hparam)
    ...
    def get_depositionMatrix_and_organName_ptsNum(self): 
        if not os.path.isdir(self.hparam.deposition_pickle_file_path):
            os.makedirs(self.hparam.deposition_pickle_file_path)
        fn_depos = os.path.join(self.hparam.deposition_pickle_file_path, 'deposition.pickle') 
        ...
        if os.path.isfile(fn_depos):
            cprint('loading deposition data.', 'green')
            D = unpickle_object(fn_depos) 
            organName_ptsNum = unpickle_object(os.path.join(self.hparam.deposition_pickle_file_path, 'organName_ptsNum.pickle'))
        else:
            cprint('build deposition data.', 'green')
            if self.hparam.dense_deposition_matrix:
                D, organName_ptsNum = self._read_deposition_file_and_save_to_pickle()
            else: # use sparse matrix to store D
                D, organName_ptsNum  = self._read_deposition_file_and_save_to_pickle_sparse()

        print(f'deposition_matrix shape={D.shape}')
        print(f'organName:ptsNum with same order of deposition.txt: \n {organName_ptsNum}; organs number={len(organName_ptsNum)}')
        return D, organName_ptsNum

    def _get_uniqueRays_doseGridNum_organOrder(self):
        print('check ray_index and organs in Deposition_Index.txt')
        ray_list, organ_order, doseGridNum = [], [], 0
        is_redundancy = False
        for line in open(self.hparam.deposition_file, 'r'):
            if 'pts_num' in line: # new organ
                pts_num = int(line.split(':')[-1])
                if pts_num == 0:
                    ...
                    organ_name = self._pointNum_organName[pts_num]
                    if organ_name in organ_order:
                        is_redundancy = True
                        cprint(f'skip duplicate points_num:{pts_num} and organ_name:{organ_name} in deposition.txt', 'yellow')
                    else:
        ...
        # organs in order, should be consistent with the deposition.txt 
        print(f'organs (in order) in deposition.txt: \n {organ_order}; organs number={len(organ_order)}')
        stop = False
        for pts_num, organ_name in self._pointNum_organName.items():
            if pts_num > 0 and organ_name not in organ_order: 
                cprint(f'non zero pts_num ({pts_num}) {organ_name} not find in deposition_Index.txt', 'red')
                stop = True
        if stop: 
            cprint('STOP running: deposition.txt seems lacking organs in csv.', 'red')
            pdb.set_trace() # deposition.txt seems lacking organs in cvs.

        self.organName_ptsNum = OrderedBunch()
        for organ_name in organ_order: self.organName_ptsNum[organ_name] = self._organName_pointNum[organ_name]
        pickle_object(os.path.join(self.hparam.deposition_pickle_file_path, 'organName_ptsNum.pickle'), self.organName_ptsNum)
    ...
    def _read_deposition_file_and_save_to_pickle_sparse(self):
        ...
        '''
        # get shape inf to build dps matirx
        ray_list, DoseGridNum = self._get_uniqueRays_doseGridNum_organOrder()
        ...
        return D, self.organName_ptsNum  # self.organName_ptsNum is create from self._get_uniqueRays_doseGridNum_organOrder; we return it here to keep code consistence 

    def _setup_paramters_from_csv_table(self):
        #  df = pd.read_csv(self.hparam.cvs_table_file, skiprows=1, index_col=0, skip_blank_lines=True) # duplicated column will be renamed automatically
        #  cols = [c for c in df.columns if 'Unnamed' not in c]
        #  df = df[cols] # drop nan columns
        #  df = df.drop('Extended_PTV', axis=1)
        df = self._parse_csv_file()

        self._organName_pointNum = pd.to_numeric(df.loc['Points Number']).to_dict(collections.OrderedDict) # OrderedDict
        print(f' organName: pointNum in csv: \n {self._organName_pointNum}; organ number ={len(self._organName_pointNum)}')

        reversed_dict = collections.OrderedDict(reversed(list(self._organName_pointNum.items())))  # reversed order to keep first column name when there are duplicated columns
        self._pointNum_organName = {value:key for key, value in reversed_dict.items()}
    ...
    def project_to_fluenceMaps(self, fluenceVector):
        '''Convert 1D fluenceVector to 2D fluenceMap
            Arguments: fluenceVector: numpy_ndarray (#bixels, )
            Return: {beam_id: numpy_ndarray fluenceMap with the shape of (H,W)} ''' 
        # set up a tmp with shape:(#bixels+1, ) and tmp[#bixels+1]=0; 
        # where #bixels+1 indicate non-valid ray.
        # In this way, we can set the intensity of nonvalid ray to 0.
        tmp = np.append(fluenceVector, 0)
    ...
    def get_rays_from_fluences(self, dict_FluenceMat):
        '''dict_fluences: {beam_id: ndarray fluence matrix}
        return:
            valid_rays: ndarray (#valid_bixels,)
        '''
        valid_rays = []
        for (_, boolMat), (idx, F) in zip(self.dict_rayBoolMat.items(), dict_FluenceMat.items()):
            valid_rays.append(F[boolMat].flatten())
    ...
    def _parse_csv_file(self):
        ...
        # this var will be used in loss.py, therefore we should keep the duplicated columns
        self.csv_table = df

        # duplicated column will be droped
        df = df.loc[:, ~df.columns.str.replace("(\.\d+)$", "").duplicated()]
    ...
    def _get_organ_3D_index(self):
        ...
        for name in sorted_df.columns:
            if name not in Dicom_Reader.all_rois:
                cprint(f'Waring: {name} not in RTStruct! we simply skip it.', 'red')
            else:
                roi_names.append(name)
        cprint(f'number of organ: {len(roi_names)}', 'green')
        ...
        # match MonteCarlo dose's shape 
        if Dicom_Reader.mask.shape != self.hparam.MCDose_shape:
            cprint(f'resize contour {Dicom_Reader.mask.shape} to match MC shape {self.hparam.MCDose_shape}', 'yellow')
            Dicom_Reader.mask = resize(Dicom_Reader.mask, self.hparam.MCDose_shape, order=0, mode='constant', cval=0, clip=False, preserve_range=True, anti_aliasing=False).astype(np.uint8)

        cprint(f'max label in label_volume = {Dicom_Reader.mask.max()}', 'green')
        cprint(f'shape of label_volume = {Dicom_Reader.mask.shape}', 'green')
    ...
    def _read_deposition_file_and_save_to_pickle(self):
        '''
        DeprecationWarning: depos matrix should always be store as sparse matrix
        '''
        # get shape inf to build dps matirx
        ray_list, points_num, organ_order = self._get_uniqueRays_pointsNum_organOrder()
        Dps_matrix = np.zeros((points_num, max(ray_list)+1), dtype=np.float32)
        print(f'organ order in depostion file:{organ_order}')

        # fill dps matrix
        print('building depostion matrix')
        with open(self.hparam.deposition_file, "r") as f:
            organ_order, points_num = [], -1
            is_redundancy = False
            for line in f:
                if 'pts_num' in line: # new organ
                    organ_name = self._pointNum_organName[float(line.split(':')[-1])]
                    if organ_name in organ_order:
                        is_redundancy = True
                    else:
                        is_redundancy = False
                        organ_order.append(organ_name)
                elif not is_redundancy and ' ]:' in line:  # new point
                    points_num += 1
                elif not is_redundancy and 'Indx' in line:  # new ray
                    ray_idx = int(line.split(' ')[0].split(':')[-1])
                    Dps_matrix[points_num, ray_idx] = float(line.split('Pt_dose:')[-1].split('(')[0])

            # organ name and pointsNum
            organ_inf = collections.OrderedDict()
            for organ_name in organ_order:
                organ_inf[organ_name] = self.organName_pointNum[organ_name]

        pickle_object(os.path.join(self.hparam.deposition_pickle_file_path, 'deposition.pickle'), [Dps_matrix, organ_inf])
        return Dps_matrix, organ_inf

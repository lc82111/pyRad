...
class PencilBeam():
    ...
    def _parse_dose_torch(self, vector_dose):
        ...
        dose = torch.zeros(doseGrid_shape, dtype=torch.float32)  # 3D dose @ doseGrid size
...
def parse_points_positions(hparam, skin_name):
    '''
    return: dose_grid coords with (points_num, 3=[x,y,z])
    '''
    with open(hparam.pointsPosition_file, "r") as f:
        lines = f.readlines()

    begin, end = np.inf, np.inf
    for i, line in enumerate(lines):
        if skin_name in line: 
            begin = i+1
        if i>=begin and ':' in line: # another organ 
            end = i
        if i>=begin and i+1==len(lines):
    ...
    # sort: z decending, y decending, x ascending  
    sort_index = np.lexsort((coords[:,0], -coords[:,1], -coords[:,2]))
    coords = coords[sort_index]
    return coords

def doseGrid_CTs_overlap(dose_grid, data):
    ## 0. prepare data
    CTs = data.Dicom_Reader.ArrayDicom 

    # slices positions in world coord
    grid_z = np.unique(dose_grid[:,-1])

    CT_zs = np.array([float(z)/10 for z in data.Dicom_Reader.slice_info])
    
    print('dose_grid slice coords:')
    print(grid_z)

    ## 1. assert z axis matching
    print('\n CT slice coords:')
    count = 0
    for ct_z in CT_zs:
        if ct_z in grid_z:
    ...
    ## 2. dose grid and CTs overlap
    for z in grid_z:
        slice_doseGrid  = dose_grid[np.where(dose_grid[:,-1]==z)]

        slice_idx = np.where(CT_zs==z)[0][0]
        
        plt.figure()
        plt.imshow(CTs[slice_idx], cmap='gray')
        plt.scatter(slice_doseGrid[:,0], slice_doseGrid[:,1], s=0.3)
        plt.show()
        plt.savefig(f'test/{z}.png')
        plt.close()

    pdb.set_trace()

def MCDose(hparam):
    with open('test/dpm_result72Ave.dat', 'rb') as f:
        data = np.fromfile(f, dtype=np.float32)
        dose = data.reshape(*hparam.MCDose_shape)
        dose = np.swapaxes(dose, 2, 1)
    return dose

def skinDose_CTs_overlap(dose_grid, data, dose_values, hparam):
    CTs = resize(data.Dicom_Reader.ArrayDicom, hparam.MCDose_shape, order=1, mode='constant', cval=0, clip=False, preserve_range=True, anti_aliasing=False)
    _, H, W = CTs.shape
    zs = np.array([float(z)/10 for z in data.Dicom_Reader.slice_info])
    ys = np.arange(0, H)
    xs = np.arange(0, W) 
    mcdose = MCDose(hparam)

    for z in zs:
        ct_zs        = np.where(zs==z)[0][0]
        slice_CT     = CTs[ct_zs]
        slice_mcdose = mcdose[ct_zs]

        grid_indexes     = np.where(dose_grid[:,-1]==z)[0]

        slice_doseGrid   = dose_grid[grid_indexes, 0:2] / 2.  # (512x512)->(256x256)
        xx               = slice_doseGrid[:,0].astype(np.int)
        yy               = slice_doseGrid[:,1].astype(np.int)

        slice_doseValues = dose_values[grid_indexes]
        grid_x, grid_y   = np.mgrid[0:W, 0:H]
        slice_interpDose = griddata(slice_doseGrid, slice_doseValues, (grid_x, grid_y), method='linear')
        slice_interpDose = slice_interpDose.T

        figs, axes = plt.subplots(1, 4, figsize=(4*10, 1*10))
        figs.set_tight_layout(True)

        axes[0].imshow(slice_CT, cmap='gray')
        slice_dose = np.zeros(slice_CT.shape) 
        slice_dose[yy, xx] = dose_values[grid_indexes] 
        _ = axes[0].imshow(slice_dose, cmap='jet', alpha=0.5)
        add_colorbar(figs, axes[0], _)

        axes[1].imshow(slice_CT, cmap='gray')
        _ = axes[1].imshow(slice_interpDose, cmap='jet', alpha=0.5)
        add_colorbar(figs, axes[1], _)

        _ = axes[2].imshow(slice_interpDose)
        add_colorbar(figs, axes[2], _)
 
        axes[3].imshow(slice_CT, cmap='gray')
        _ = axes[3].imshow(slice_mcdose, cmap='jet', alpha=0.5)
        add_colorbar(figs, axes[3], _)

        plt.show()
        plt.savefig(f'test/{z}_beam1_{slice_doseValues.max()}.png')
        plt.close()
    print('done')

def cal_dose(data, skin_name):
    from skimage.draw import disk, set_color

    # test
    num_ray = 0
    for beam_id, FM in data.dict_rayBoolMat.copy().items():
        num_ray += FM.sum()
    #  print(num_ray)
    assert num_ray == data.deposition.shape[1]

    # set up a random fake beam in one angle 
    random_beam_id = np.random.randint(0, data.num_beams)

    dict_fake_FMs = OrderedBunch()
    for beam_id, FM in data.dict_rayBoolMat.copy().items():
        if beam_id == random_beam_id:
            #  rr, cc = disk((10,10), 5, shape=FM.shape)
            beam = np.zeros(FM.shape)
            #  beam[rr, cc] = 1
            #beam[13, 14] = 1
            #  dict_fake_FMs[beam_id] = beam*13
            dict_fake_FMs[beam_id] = FM*13
        else:
            dict_fake_FMs[beam_id] = FM*0
    fluence_vector = data.get_rays_from_fluences(dict_fake_FMs)  

    #fluence = np.ones((D.shape[1], 1), dtype=np.float32)
    dose = data.deposition.dot(fluence_vector)
    skin_dose = dose[0:data.organName_ptsNum[skin_name]]
    return skin_dose


class PencilBeamBak():
    def __init__(self, hparam, data, roi_skinName='ITV_skin', fill_value=np.nan):
        self.hparam = hparam
        self.roi_skinName = roi_skinName 
        self.data = data
        self.fill_value = fill_value
        self.spatial_scale_factor = 2.  # (512x512)->(256x256); TODO: juyao may change dose_grid to a different scale 

        # get dose grid coords
        self.dose_grid = self._parse_points_positions() # @ 126x512x512
        self._change_doseGrid_zs() #  change 
    ...
    def _set_skin_coords(self): 
        '''get corner of maximum skin rectangle  '''
        dose_grid = self.dose_grid[:, 0:2] / self.spatial_scale_factor
        dose_grid = dose_grid.round().astype(np.uint)

        self.skin_lefttop = OrderedBunch({'x':dose_grid[:,0].min(), 'y':dose_grid[:,1].min()})
        self.skin_rightbot= OrderedBunch({'x':dose_grid[:,0].max(), 'y':dose_grid[:,1].max()})

    def _parse_points_positions(self):
        '''
        get dose_grid coords, from pointsPosition.txt
        return: coords with shape (points_num, 3=[x,y,z])
        '''
        with open(self.hparam.pointsPosition_file, "r") as f:
            lines = f.readlines()
    ...
    def _change_doseGrid_zs(self):
        '''change zs from world coords e.g. 3.6 to image coords e.g. 0'''
        zs = np.array([float(z)/10 for z in self.data.Dicom_Reader.slice_info])  # z axis coords in the world coord 
        for i, z in enumerate(zs): # for each slice
            grid_indexes = np.where(self.dose_grid[:,-1]==z)[0]
            self.dose_grid[grid_indexes, -1] = i
        # xyz -> zyx @  63x256x256
        dose_grid_rescaled = (self.dose_grid / 2).round()
        self.doseGrid_zz_yy_xx = (dose_grid_rescaled[:,2], dose_grid_rescaled[:,1], dose_grid_rescaled[:,0])

    def _parse_dose(self, vector_dose):
        ''' turn 1D dose to 3D and interp the 3D dose
        return: dose (#slice, H=256, W=256) '''
        _, H, W = self.hparam.MCDose_shape
        zs = np.array([float(z)/10 for z in self.data.Dicom_Reader.slice_info])  # z axis coords in the world coord 
        dose = np.empty(self.hparam.MCDose_shape, dtype=np.float32)
        for i, z in enumerate(zs): # for each slice
            grid_indexes     = np.where(self.dose_grid[:,-1]==i)[0]
            slice_doseGrid   = self.dose_grid[grid_indexes, 0:2] / self.spatial_scale_factor  # (512x512)->(256x256); TODO: juyao may change dose_grid to a different scale 
            slice_doseValues = vector_dose[grid_indexes]
            grid_x, grid_y   = np.mgrid[0:W, 0:H]
            slice_interpDose = griddata(slice_doseGrid, slice_doseValues, (grid_x, grid_y), method='linear', fill_value=self.fill_value)  # NOTE: griddata will fill nan for requested points outside of the convex hull of the input points.
            dose[i]          = slice_interpDose.T.astype(np.float32)
        dose = np.where(dose<0, 0, dose)  # TODO: is this right? seems right to me.
        return dose 

    def _parse_dose_without_interp_torch(self, vector_dose):
        ''' turn 1D dose to 3D.
        Arguments: vector_dose tensor
        Return: dose (D=63, H=256, W=256) '''
        with torch.no_grad():
            D, H, W = self.hparam.MCDose_shape 
            dose = torch.zeros((D//2, H, W), dtype=torch.float32)  # 3D dose 63x256x256
            assert len(vector_dose) == len(self.dose_grid)
            dose[self.doseGrid_zz_yy_xx] = vector_dose  #  63x256x256
            #  dose = torch.nn.functional.interpolate(dose.view(1,1,D,H*2,W*2), size=(D//2,H,W), mode='trilinear', align_corners=False)
            #  dose = torch.nn.functional.interpolate(dose.view(1,1,D,H*2,W*2), size=(D//2,H,W), mode='nearest')
            return dose

    def get_dose(self, uid, is_interp=True):
        ''' return:pbDose (#slice, H, W) '''
        # get saved fluence of beam_id and aperture_id
        beam_id, apert_id = uid.split('_')
        beam_id, apert_id = int(beam_id), int(apert_id)
        ...
        fluence_vector = self.data.get_rays_from_fluences(dict_FMs)  
        dose = self.data.deposition.dot(fluence_vector)
        vector_dose = dose[0:self.data.organName_ptsNum[self.roi_skinName]]
        if is_interp:
            pbDose = self._parse_dose(vector_dose) # 3D dose 126x256x256
        else:
            vector_dose = torch.tensor(vector_dose, dtype=torch.float32)
            pbDose = self._parse_dose_without_interp_torch(vector_dose).cpu().numpy() # 3D dose 63x256x256
            pbDose = np.squeeze(pbDose).astype(np.float32)
        return pbDose
